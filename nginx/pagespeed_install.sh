    ...
    if [ -f /etc/debian_version ]; then
      ...
      if gcc_too_old; then
        if [ ! -e /usr/lib/gcc-mozilla/bin/gcc ]; then
          ...
          run apt-get install ${INSTALL_FLAGS} gcc-mozilla
        fi
...
$(cat /etc/redhat-release) Expected 5 or 6."
          ...
          run wget "$slc_key" -O "$slc_key_out"
          run rpm --import "$slc_key_out"
          ...
          run wget -O "$repo_fname" "$repo_url"
          run yum install ${INSTALL_FLAGS} devtoolset-2-gcc-c++ devtoolset-2-binutils
        fi
